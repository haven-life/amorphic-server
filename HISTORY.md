## 4.2.0
* Adding possible `serverMode` app level config option for amorphic. This key, if it exists has only one value that can be associated with it, `daemonOnly`. This marks this application as **daemon exclusively**. This is provided as an **alternative** to the already existing `isDaemon`, and should be adopted for daemon applications going forward.
...
* `isDaemon` should be reserved for Legacy applications from now on. It is suggested to use `serverMode: "daemonOnly"` for daemon specific applications going forward
* It is **not recommended** to use both `serverMode` and `isDaemon` as possible configurations. If both `serverMode` is set to `daemonOnly` and `isDaemon` is set to true, then amorphic will default to `daemonOnly`.
* All api routes will be continue to be namespaced by default the `/api` endpoint if `isDaemon` is set to true. If an app is set to `serverMode: "daemonOnly"`, all routes will be namespaced by default to `/`. In addition, it is possible to specify a namespace value if you would like. Within your app-level config.json add an additional key for `serverOptions` as an object and within that object add an addition key called `apiPath`, with whatever string value necessary. You can see an example of this in the `daemon_only_apiPath` test
