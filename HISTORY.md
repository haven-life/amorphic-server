## 4.2.0
* Adding possible 'daemonOnly' config value for the 'isDaemon' key in the config.json. This sets
Daemon applications to use /amorphic routes (the regular amorphic routes) by default, in addition
to their /api endpoints, but also allows applications that are daemon/api only to choose not to serve up the /amorphic route
## 4.1.0
