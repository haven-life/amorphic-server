...
function listen(appDirectory, sessionStore, preSessionInject, postSessionInject, sendToLogFunction, statsdClient) {
    ...
    // if we are being passed in a statsd client, and we've enabled logging via config value, pass it through.
    if (statsdClient && amorphicOptions.amorphicEnableStatsd) {
        // @TODO add an exportable type to this
        // @TODO add some helpful validations if what's supplied doesn't fit the spec.
        AmorphicContext.appContext.statsdClient = statsdClient;
    }
