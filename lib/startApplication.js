...
function finishDaemonIfNeeded(config, prop, prefix, appName, baseTemplate, appTemplates) {
    let ControllerTemplate = AmorphicContext.applicationTSController[appName] || appTemplates[prop].Controller;
	...
	if (config.isDaemon) {
        ...
        // @async 
        baseTemplate.controller.onAppStartup();
		logMessage(appName + ' started as a daemon');
    }
    else {
        // @async
        // @TODO: hmm... where does onAppStartup lie?
        // Bluebird.resolve(controller.onAppStartup());
        debugger;
    }
}
